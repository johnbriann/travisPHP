...
class Home extends CI_Controller {
	...
	public function dashboard(){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			$total_income = $this->db->select('SUM(order_total) as total_income')->where('status','complete')->get('orders')->row_array();
			$total_orders = $this->db->where('status','complete')->count_all_results('orders');
			$total_products = $this->db->where('status',1)->count_all_results('product');
			$total_pending_orders = $this->db->where('status','complete')->where('order_status','pending')->count_all_results('orders');
			$total_completed_orders = $this->db->where('status','complete')->where('order_status','delivered')->count_all_results('orders');
			$total_returned_orders = $this->db->where('returned',1)->count_all_results('orders');
			$total_customers = $this->db->where('status',1)->count_all_results('customer');
			$data['total_income'] = $total_income['total_income'];
			$data['total_orders'] = $total_orders;
			$data['total_products'] = $total_products;
			$data['total_pending_orders'] = $total_pending_orders;
			$data['total_completed_orders'] = $total_completed_orders;
			$data['total_returned_orders'] = $total_returned_orders;
	...
	public function products($param = NULL, $slug = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			...
			foreach($content_data as $key => $row){
				$stockData = $this->db->select('SUM(quantity) as stock')->where('product_id',$row['id'])->group_by('product_id')->get('product_stock_availability')->row_array();
				$content_data[$key]['stock'] = $stockData['stock'];
			}
			...
			if($this->input->post()){
				...
				if(!empty($insert_array)){
					...
					if($insert){
						...
						*/
						if($postData['stock_available'] != ''){
							$stock_array = array(
								'product_id' => $product_id,
								'quantity' => $postData['stock_available'],
								'date_created' => date('Y-m-d H:i:s'),
							);
							$insert = $this->db->insert('product_stock_availability',$stock_array);
						}
...
/*------------------------STOCK---------------------------*/
/*------------------------------------------------------*/
	public function stock($param = NULL, $slug = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			$this->db->select('product.id,product.name,product.image,product_stock_availability.quantity');
			$this->db->join('product','product.id = product_stock_availability.product_id');
			$stock = $this->db->get('product_stock_availability')->result_array();

			$data['content_data'] = $stock;
			$data['page_title'] = 'Product Stock';
			$data['page_content'] = 'product/list_stock';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function add_stock($product_id = NULL){
		...
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			...
			$data['products'] = $this->db->select('id, name')->order_by('name','ASC')->get('product')->result_array();
			$data['page_title'] = 'Add Product Stock';
			$data['page_content'] = 'product/add_stock';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function add_stock_process(){
		// echo "<pre>";print_r($_POST);echo "</pre>";exit;
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			if($this->input->post()){
				$postData = $this->security->xss_clean($this->input->post());
				$insert_array['product_id'] = $postData['product_id'];
				$insert_array['quantity'] = $postData['quantity'];
				$stockData = $this->db->where('product_id',$postData['product_id'])->get('product_stock_availability')->row_array();
				if(empty($stockData)){
					$stock_array = array(
					...
					$insert = $this->db->insert('product_stock_availability',$stock_array);
					$stockData = $this->db->where('product_id',$this->db->insert_id())->get('product_stock_availability')->row_array();
				}
				if(!empty($stockData)){
					if($postData['quantity'] < 0){
						if($stockData['quantity'] < $postData['quantity']){
							$this->session->set_flashdata('msg','Removal quantity cannot be less then available quantity');
							redirect('add_stock/'.$postData['product_id']);
						}else{
							$remove = 1;
							$new_quantity = $stockData['quantity']+$postData['quantity'];
						}
					}else{
						$remove = 0;
						$new_quantity = $stockData['quantity']+$postData['quantity'];
					...
					$update = $this->db->where('product_id',$postData['product_id'])->update('product_stock_availability',$update_array);
					if($update){
						$stock_reill_array = array(
							'stock_id' => $stockData['id'],
							'refil_amount' => $postData['quantity'],
							'remove' => $remove,
							'employee_id' => $this->session->userdata('userid'),
							'date_created' => date('Y-m-d H:i:s'),
						);
						$insert = $this->db->insert('stock_availability_refil',$stock_reill_array);
						$this->session->set_flashdata('msg','Stock Updated');
						redirect('stock');
					}else{
						$this->session->set_flashdata('msg','Something went wrong');
						redirect('add_stock/'.$postData['product_id']);
	...
	public function add_product_images1($hidden_id = NULL){
		// echo "<pre>";print_r($_FILES);echo "</pre>";exit;
		if (!empty($_FILES['file'])) {
			$count = count($_FILES['file']['name']);
	...
	public function edit_product_process1($id = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			if($id != NULL){
				if($this->input->post()){
					$postData = $this->security->xss_clean($this->input->post());
...
/*------------------------------------------------------*/
	public function orders($param = NULL,$type = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			$this->db->select('orders.*,customer.first_name, customer.last_name');
			if($param == 'total_orders'){
				$this->db->where('orders.order_date >=', date('Y-m-d').' 00:00:00');
				$this->db->where('orders.order_date <=', date('Y-m-d').' 23:59:59');
				$this->db->where('orders.type !=','');
			}elseif($param == 'dinein_orders'){
				$this->db->where('orders.order_date >=', date('Y-m-d').' 00:00:00');
				$this->db->where('orders.order_date <=', date('Y-m-d').' 23:59:59');
				$this->db->where('orders.type','dine-in');
			}elseif($param == 'takeaway_orders'){
				$this->db->where('orders.order_date >=', date('Y-m-d').' 00:00:00');
				$this->db->where('orders.order_date <=', date('Y-m-d').' 23:59:59');
				$this->db->where('orders.type','takeaway');
			}elseif($param == 'delivery_orders'){
				$this->db->where('orders.order_date >=', date('Y-m-d').' 00:00:00');
				$this->db->where('orders.order_date <=', date('Y-m-d').' 23:59:59');
				$this->db->where('orders.type','delivery');
			}elseif($param == 'incomplete'){
				$this->db->where('orders.status','incomplete');
			}elseif($param == 'returned'){
				$this->db->where('orders.returned',1);
			}elseif($param == 'rejected'){
				$this->db->where('orders.accepted',2);
				$this->db->where('orders.status','complete');
				$this->db->where('orders.order_status','rejected');
			}elseif($param == 'canceled'){
				$this->db->where('orders.accepted',2);
				$this->db->where('orders.status','complete');
				$this->db->where('orders.order_status','canceled');
			}else{
				// $this->db->where('orders.accepted <',2);
				$this->db->where('orders.status','complete');
				$this->db->where('orders.order_status',$param);
			}
			$this->db->join('customer','customer.id = orders.customer_id','left');
			$data['orders'] = $this->db->order_by('orders.id','DESC')->get('orders')->result_array();
			$data['page_title'] = ucwords($param).' Orders';
			$data['page_content'] = 'orders/list';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function order_detail($order_id = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			if($order_id != NULL){
				$data['order'] = $this->db->where('id',$order_id)->get('orders')->row_array();
				if(!empty($data['order'])){
					$data['customer'] = $this->db->where('id',$data['order']['customer_id'])->get('customer')->row_array();
					$data['order_billing_address'] = $this->db->where('order_id',$data['order']['id'])->get('order_billing_address')->row_array();
					$data['order_shipping_address'] = $this->db->where('order_id',$data['order']['id'])->get('order_shipping_address')->row_array();
					...
					$data['page_content'] = 'orders/detail';
					$this->load->view('common/main_view',$data);
				}else{
		...
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			...
			if($id != NULL){
				if($param != NULL){
					...
					if($param == 'complete'){
						$insert_array['order_status'] = 'delivered';
						$insert_array['delivery_date'] = date('Y-m-d H:i:s');
					}
					if($param == 'cancel'){
						$insert_array['accepted'] = 2;
	...
	public function customers(){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			...
			foreach($customers as $key => $customer){
				$customers[$key]['total_orders'] = $this->db->where('customer_id',$customer['id'])->count_all_results('orders');
			}
			$data['content_data'] = $customers;
			$data['page_title'] = 'Customers';
			$data['page_content'] = 'customers/list';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
...
/*------------------------------------------------------*/
	public function homepage_slider(){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			$data['content_data'] = $this->db->order_by('id','ASC')->get('homepage_slider')->result_array();
			$data['page_title'] = 'Homepage Slider';
			$data['page_content'] = 'pages/homepage_slider';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function add_homepage_slider(){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			// $data['content_data'] = $this->db->order_by('id','ASC')->get('homepage_slider')->result_array();
			$data['page_title'] = 'Add Homepage Slider';
			$data['page_content'] = 'pages/add_homepage_slider';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function edit_homepage_slider($id = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			$data = array();
			$data['content_data'] = $this->db->where('id',$id)->get('homepage_slider')->row_array();
			$data['page_title'] = 'Edit Homepage Slider';
			$data['page_content'] = 'pages/edit_homepage_slider';
			$this->load->view('common/main_view',$data);
		}else{
			$this->session->set_flashdata('msg','Please Login');
			redirect();
	...
	public function add_homepage_slider_process($id = NULL){
		// echo "<pre>";print_r($_POST);echo "</pre>";exit;
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			// if($id != NULL){
				if($this->input->post()){
					$postData = $this->security->xss_clean($this->input->post());
					$insert_array['title'] = $postData['title'];
					$insert_array['description'] = $postData['description'];
					$insert_array['image'] = str_replace('small_','',$postData['image_hidden']);
					// $insert_array['image'] = str_replace('small_','',$postData['image_box_hidden']);
					$insert_array['date_created'] = date('Y-m-d H:i:s');
					if(!empty($insert_array)){
						$update = $this->db->insert('homepage_slider',$insert_array);
						if($update){
							$this->session->set_flashdata('okay_msg','Slider Updated');
							redirect('homepage_slider');
						}
					}else{
						$this->session->set_flashdata('msg','Something went wrong');
						redirect('homepage_slider');
	...
	public function edit_homepage_slider_process($id = NULL){
		// echo "<pre>";print_r($_POST);echo "</pre>";exit;
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			if($id != NULL){
				if($this->input->post()){
					...
					$insert_array['date_created'] = date('Y-m-d H:i:s');
					if(!empty($insert_array)){
						$update = $this->db->where('id',$id)->update('homepage_slider',$insert_array);
						if($update){
							$this->session->set_flashdata('okay_msg','Slider Updated');
							redirect('homepage_slider');
						}
	...
	public function delete_homepage_slider($id = NULL){
		if($this->session->userdata('userid') && $this->session->userdata('is_admin') == 'yes'){
			if($id != NULL){
				$ifExists = $this->db->where('id',$id)->count_all_results('homepage_slider');
				if($ifExists > 0){
					$delete = $this->db->where('id',$id)->delete('homepage_slider');
					if($delete){
						$this->session->set_flashdata('okay_msg','Slider Deleted');
						redirect('homepage_slider');
					}else{
						$this->session->set_flashdata('msg','Something went wrong');
						redirect('homepage_slider');
