...
try{
	...
	if($californiabuttonclick){
		...
		if($morefilterbuttonclick){
			sleep(1);
			$checkbox_fishing = $web_driver->findElement(WebDriverBy::xpath("//or='Trolling']"))->click();
			$checkbox_fishing = $web_driver->findElement(WebDriverBy::xpath("//label[@flabel[@for='Kite_Fishing']"))->click();
