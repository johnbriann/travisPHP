					...
					if($button_calendarnextmonthclick){
						...
						if($button_calendardateclick){
							...
							if($button_calendarbookingclick){
								sleep(1);
								$dropdown_tripname = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_trip_id"))->findElement(WebDriverBy::cssSelector("option[value='37']"))->click();
								if($dropdown_tripname){
									sleep(2);
									$dropdown_tripduration = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_trip_rates"))->findElement(WebDriverBy::cssSelector("option[value='320']"))->click();
									if($dropdown_tripduration){
										sleep(2);
										$dropdown_triptime = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_trip_time"))->findElement(WebDriverBy::cssSelector("option[value='12:00']"))->click();
										if($dropdown_triptime){
											// sleep(2);
											$dropdown_tripguest = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_trip_guests"))->findElement(WebDriverBy::cssSelector("option[value='2']"))->click();
											if($dropdown_tripguest){
												// sleep(2);
												$input_tripnotes = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_trip_comments"))->sendKeys("Lorem Ipsum is simply dummy text of the printing and typesetting industry. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s, when an unknown printer took a galley of type and scrambled it to make a type specimen book. It has survived not only five centuries, but also the leap into electronic typesetting, remaining essentially unchanged. It was popularised in the 1960s with the release of Letraset sheets containing Lorem Ipsum passages, and more recently with desktop publishing software like Aldus PageMaker including versions of Lorem Ipsum.");
												$input_tripcustomername = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_customer_name"))->sendKeys("John Doe");
												$input_tripcustomeremail = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_customer_email"))->sendKeys("testcustomer".$random_date."@gmail.com");
												$input_tripcustomermobile = $web_driver->findElement(WebDriverBy::cssSelector("#normal_booking_customer_mobile"))->sendKeys("1234567890");
												$button_calendarbookingokay = $web_driver->findElement(WebDriverBy::cssSelector("#normal-booking-save-button"))->click();
