		...
		if($element2){
			...
			if($submit_pass){
				...
				if($reportbuttonnavclick){
					sleep(1);
					$editreportbuttonclick = $web_driver->findElement(WebDriverBy::cssSelector("#guide-first-report"))->click();
					if($editreportbuttonclick){
						sleep(1);
						$image = file_get_contents('https://yentna.com/assets/images/logo.png');
						$image_put = file_put_contents("C:\\Users\\yentna.png", $image);
						if($image_put){
							$enter_report_title = $web_driver->findElement(WebDriverBy::cssSelector("#report_title"))->sendKeys(" Edit");
							$enter_address_empty = $web_driver->executeScript("document.getElementById('report_address').value='';");
							...
							$enter_desc = $web_driver->findElement(WebDriverBy::cssSelector("#report_desc"))->sendKeys(" Edit");
							$addreportsubmitbuttonclick = $web_driver->findElement(WebDriverBy::cssSelector(".savebtn"))->click();
